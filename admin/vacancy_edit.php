...
session_start();
$vid = $_GET['vacancy_id'];
$ssql ="select * from vacancy where vacancy.id='$_GET[vacancy_id]'";
...
if(isset($_POST['submit'])) {
    ...
    if($extension == 'jpg'||$extension == 'png'||$extension == 'gif') {        
        if ($fsize>=1000000) {
            ...
            $sql = "update vacancy set title ='$_POST[v_title]', price ='$_POST[v_price]', claim ='$_POST[v_claim]', abligation ='$_POST[v_obligation]', img='$fnew' where id='$_GET[vacancy_id]'";
    ...
    header("location:vacancy_edit.php?vacancy_id=$vid");
}
?>
<head>
    <meta charset="UTF-8">
        ...
        <div class="wrapper">
            <form class="form" action='' method='post' enctype="multipart/form-data">
                <div class="form__item short-form">
                    ...
                    <input type="text" name="v_title" id="title" value="<?=$row['title']?>">
                </div>
                <div class="form__item short-form">
                    <label for="price">Заработная плата</label>
                    <input type="number" name="v_price" id="price" value="<?=$row['price']?>">
                    <div class="fixed-value">руб.</div>
                </div>
                <div class="form__item">
                    <label for="claim">Требования</label>
                    <textarea name="v_claim" id="claim"><?=$row['claim']?></textarea>
                </div>
                <div class="form__item">
                    <label for="obligation">Обязанности</label>
                    <textarea name="v_obligation" id="obligation"><?=$row['obligation']?></textarea>
                </div>
                <div class="form__item">
