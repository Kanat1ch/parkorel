...
session_start();
$cid = $_GET['club_id'];
$ssql ="select * from clubs where clubs.id='$_GET[club_id]'";
...
if(isset($_POST['submit'])) {
    ...
    if($extension == 'jpg'||$extension == 'png'||$extension == 'gif') {        
        if ($fsize>=1000000) {
            ...
            $sql = "update clubs set title ='$_POST[c_title]', scontent ='$_POST[c_scontent]', director ='$_POST[c_director]', phone ='$_POST[c_phone]', content ='$_POST[c_content]',  img='$fnew' where id='$_GET[club_id]'";
    ...
    header("location:club_edit.php?club_id=$cid");
}
?>
<head>
    <meta charset="UTF-8">
        ...
        <div class="wrapper">
            <form class="form" action='' method='post' enctype="multipart/form-data">
                <div class="form__item">
                    ...
                    <input type="text" name="c_title" id="title" value="<?=$row['title']?>">
                </div>
                <div class="form__item">
                    <label for="short-content">Краткое описание <small style="color: #777;">(Максимум 200 символов)</small></label>
                    <input type="text" name="c_scontent" id="short-content" maxlength="200" value="<?=$row['scontent']?>">
                </div>
                <div class="form__item short-form">
                    <label for="director">Руководитель</label>
                    <input type="text" name="c_director" id="director" value="<?=$row['director']?>">
                </div>
                <div class="form__item short-form">
                    <label for="phone">Телефон</label>
                    <input type="text" name="c_phone" id="phone" value="<?=$row['phone']?>">
                </div>
                <div class="form__item">
                    <label for="content">Описание</label>
                    <textarea name="c_content" id="content"><?=$row['content']?></textarea>
                </div>
                <div class="form__item">
